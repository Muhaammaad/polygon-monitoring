...
# LoginCheck implementation
class LoginCheck(BaseCheck):
    """Check for drivers logging in inside/outside designated login zones."""
    ...
    # Helper to get login statuses
    def _get_login_statuses(self, city_config: Any, check_cfg: Any) -> List[str]:
        """Resolve raw DB statuses for the configured login status group."""
    ...
    def query(self, city_config: Any) -> List[Dict[str, Any]]:
        """Query recent login events that qualify as new logins."""
        check_cfg = self._get_check_config(city_config, 'login_check')
        if not check_cfg:
            return []   
        ...
        if not time_window:
            ...
            logger.error(f"login_check missing time_window_minutes in {city_code}")
            return []

        excluded_minutes = getattr(check_cfg, 'excluded_minutes', None)
        if excluded_minutes is None and isinstance(check_cfg, dict):
            excluded_minutes = check_cfg.get('excluded_minutes', 120)
        if not excluded_minutes:
            excluded_minutes = 120

        raw_statuses = self._get_login_statuses(city_config, check_cfg)
        if not raw_statuses:
            city_code = getattr(city_config, 'city_code', 'UNKNOWN')
            logger.info(f"No statuses mapped for login_check in {city_code}")
            return []
        ...
        query = f"""
            ...
            AND NOT EXISTS (
                ...
                AND t2.ts BETWEEN DATE_SUB(NOW(), INTERVAL %s MINUTE)
                    AND DATE_SUB(NOW(), INTERVAL %s MINUTE)
            )
        """

        try:
            cursor = self.db.cursor(pymysql.cursors.DictCursor)
            params = [time_window, city_config.city_code] + raw_statuses + [excluded_minutes, time_window]
            cursor.execute(query, params)    
            results = cursor.fetchall()   
            cursor.close()  
            return results if results else []
        except Exception as e:
            logger.error(f"Error querying login candidates: {e}")
            return []

    # Evaluate candidate
    def evaluate(self, candidate: Dict[str, Any], city_config: Any) -> Optional[str]:
        # Evaluate a candidate's login location relative to the login zone.
        check_cfg = self._get_check_config(city_config, 'login_check')
        if not check_cfg:
            return None
        ...
        target_zone = target_zone or 'login_zone'
        ...
        try:
            ...
            return 'login_inside_zone' if in_zone else 'login_outside_zone'
        except Exception as e:
            logger.error(f"Error evaluating login candidate {candidate.get('uuid')}: {e}")
            return None

    # Execute check
    def execute(self, city_config: Any) -> List[Dict[str, Any]]:
        """Run the login check workflow for a single city."""
