...
# CenterCheck implementation
class CenterCheck(BaseCheck):
    """Check for drivers paused inside the waiting/center zone."""
    ...
    # Helper to get center statuses
    def _get_center_statuses(self, city_config: Any, check_cfg: Any) -> List[str]:
        """Resolve raw DB statuses for the configured center/waiting group."""
    ...
    def query(self, city_config: Any) -> List[Dict[str, Any]]:
        """Query recent activity for drivers that should be in center/waiting zones."""
        check_cfg = self._get_check_config(city_config, 'center_check')
        if not check_cfg:
            return []
        ...
        if not time_window:
            ...
            logger.error(f"center_check missing time_window_minutes in {city_code}")
            return []

        raw_statuses = self._get_center_statuses(city_config, check_cfg)
        if not raw_statuses:
            city_code = getattr(city_config, 'city_code', 'UNKNOWN')
            logger.info(f"No statuses mapped for center_check in {city_code}")
            return []
        ...
        try:
            ...
            logger.error(f"Error querying center candidates: {e}")
            return []

    # Evaluate candidate
    def evaluate(self, candidate: Dict[str, Any], city_config: Any) -> Optional[str]:
        """Evaluate if driver is inside the configured waiting/center zone."""
        check_cfg = self._get_check_config(city_config, 'center_check')
        if not check_cfg:      
            return None     
        ...
        target_zone = target_zone or 'waiting_zone'
        ...
        try:
            ...
            if not in_zone:
                logger.debug(f"Driver {candidate.get('uuid')} waiting outside {target_zone}")
                return 'not_waiting_in_center'     
            return None        
        except Exception as e:
            logger.error(f"Error evaluating center check candidate {candidate.get('uuid')}: {e}")
            return None
        
    # Execute check
    def execute(self, city_config: Any) -> List[Dict[str, Any]]:
        """Run the center/waiting check workflow for a single city."""
        ...
        warnings: List[Dict[str, Any]] = []   # Accumulate warnings generated during evaluation
